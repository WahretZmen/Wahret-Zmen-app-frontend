...
const canonicalCategory = (raw) => {
  ...
  if (["children", "kid", "kids", "enfant", "enfants", "أطفال"].includes(k)) {
    return "children";
  }
...
const canonicalSubCategory = (raw) => {
  ...
  if (
    ["accessories", "accessory", "accessoires", "accessoire", "إكسسوارات", "اكسسوارات"].includes(
      k
    )
  ) {
    return "accessories";
  ...
  if (["costume", "بدلة"].includes(k)) return "costume";
  if (["صدريّة", "صدرية"].includes(k)) return "صدريّة";
  if (["عباية رجالي", "عباية رجالية"].includes(k)) return "عباية رجالي";
  if (["جبة", "جبّة", "jebba"].includes(k)) return "جبة";
...
const OurSellers = () => {
  ...
  const filteredProducts = useMemo(() => {
    const selectedCat = canonicalCategory(selectedCategory);
    const selectedSub = canonicalSubCategory(selectedSubCategory);
    ...
    const categoryFiltered =
      !selectedCat
        ...
        : products.filter(
            (product) => canonicalCategory(product?.category) === selectedCat
          );

    if (!selectedSub) return categoryFiltered;

    return categoryFiltered.filter(
      (product) => canonicalSubCategory(getProductSubCategory(product)) === selectedSub
    );
