    ...
    <header className="navbar-container" ref={headerRef}>
      <nav className="navbar-content" dir={dir}>
        ...
        <div className="navbar-left">
          <Link to="/" className="logo premium-logo" onClick={() => setIsMobileMenuOpen(false)}>
        ...
        <ul className={`nav-links ${isMobileMenuOpen ? "mobile-center open" : ""}`}>
          ...
          {token && (
            <li><Link to="/dashboard" onClick={() => setIsMobileMenuOpen(false)}>{t("admin_dashboard")}</Link></li>
          )}
        </ul>

        {/* Right icons */}
        <div className="nav-icons">
          <Link to="/cart" className="cart-icon" onClick={() => window.scrollTo(0, 0)} aria-label={t("cart")}>
          ...
          {currentUser ? (
            <div className="user-menu" ref={dropdownRef}>
              <button
                ...
                onClick={(e) => { e.stopPropagation(); setIsDropdownOpen((s) => !s); }}
              >
              ...
              >
                <ul>
                  <li role="menuitem"><Link to="/user-dashboard" onClick={() => setIsDropdownOpen(false)}>{t("dashboard")}</Link></li>
                  <li role="menuitem"><Link to="/orders" onClick={() => setIsDropdownOpen(false)}>{t("orders")}</Link></li>
                  <li role="menuitem">
                    <button type="button" onClick={() => { logout(); setIsDropdownOpen(false); navigate("/"); }}>
                      {t("logout")}
                    </button>
                  </li>
                </ul>
            ...
            <Link to="/dashboard" className="dashboard-link admin-only">{t("admin_dashboard")}</Link>
          ) : (
            <Link to="/login" className="login-icon" aria-label={t("login")}><FiUser className="icon" /></Link>
          )}
        </div>
      </nav>
    </header>
