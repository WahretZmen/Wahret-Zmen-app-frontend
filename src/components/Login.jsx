  ...
  const handleGoogleSignIn = async (e) => {
    ...
    try {
      ...
      if (res?.status === "ok") {
        ...
        // Do nothing; after redirect back, auth state will update
      } else if (res?.status === "cancel") {
        // User closed the popup → no success, no navigate
      } else {
        // Unexpected shape
        console.warn("[Login] Unexpected Google sign-in status:", res);
      }
    } catch (error) {
      // Config or other hard errors
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-[#F4EEE0]">
      <div className="w-full max-w-md bg-white p-8 shadow-lg rounded-lg">
        <h2 className="text-[#8B5C3E] text-2xl font-semibold text-center mb-4">{t("login.title")}</h2>
        ...
        <p className="text-center text-sm text-gray-700 mt-3">
          {t("login.no_account")}{" "}
          <Link to="/register" className="text-[#8B5C3E] hover:underline">{t("login.register_link")}</Link>
        </p>
