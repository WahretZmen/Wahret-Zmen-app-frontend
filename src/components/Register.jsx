...
const Register = () => {
  ...
  const { currentUser, registerUser, signInWithGoogle, isGoogleSigningIn } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (res?.status === "cancel") {
        setAwaitingGoogle(false); // user closed popup
  ...
  // Only succeed after Google if we were awaiting and a user actually appeared
  useEffect(() => {
      ...
      <div className="w-full max-w-md bg-white p-8 shadow-lg rounded-lg">
        ...
        <div className="text-center mt-4">
          ...
          >
            ...
            {(isGoogleSigningIn || awaitingGoogle) ? t("register.google_btn") + "..." : t("register.google_btn")}
          </button>
        </div>
